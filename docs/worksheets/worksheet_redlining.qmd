...
```{r, cache=TRUE}
Denver_police_shootings_cloud <- create_wordclouds_by_grade(Denver_police_shootings_match$sf, output_file = "police_shootings_word_cloud_per_grade.png",title = "police involved shooting per crime type where larger text is more frequent", max_size =35, col_select = "SHOOT_ACTI")
...
process_tree_inventory_data <- function(address, inner_file, polygon_layer, output_filename) {
  ...
  return(list(plot = plot, tree_layer = trees))
}
...
```{r}
result <- process_tree_inventory_data(
