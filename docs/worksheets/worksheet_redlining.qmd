...
sf_data_filtered <- denver_redlining %>% 
...
ggplot(data = sf_data_filtered) +
  ...
  geom_sf(data = rivers, col="blue", alpha=0.1, lwd=1.1) +
...
process_satellite_data <- function(polygon_layer, start_date, end_date, assets, fps = 4, output_file = "anim.gif") {
  # Record start time
  start_time <- Sys.time()
  ...
  raster_cube(col, v, mask = S2.mask) %>%
    ...
    animate(col = ndvi_col, zlim = c(-0.2, 1), key.pos = 1, save_as = output_file, fps = fps)
...
# Example usage:
# Assuming you have a proper 'polygon_layer' loaded as an sf object, and 'assets' defined
 processing_time <- process_satellite_data(denver_redlining, "2020-01-01", "2022-12-31", c("B04", "B08"))
